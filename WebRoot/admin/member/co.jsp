...
function top(){
   	form3.action="<%=basePath%><%=dir %>/member/co.jsp?page=1";
    form3.submit();
}
function last(){
	...
	form3.action="<%=basePath%><%=dir %>/member/co.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%><%=dir %>/member/co.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%><%=dir %>/member/co.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%><%=dir %>/member/co.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%><%=dir %>/member/co.jsp?page="+page;
      form3.submit();
  ...
  <tr bgcolor='#FFFFFF'> 
    <td bgcolor="#FFFFFF"> 
       ...
       <TABLE width="100%" border=0 align="center" cellPadding=3 cellSpacing=1 class=tablewidth>
		...
		<%
			...
			int cou = mb.getMessageCountCO();//得到信息总数			        
			...
			List pagelist1 = mb.getMessageCO(Integer.parseInt(page1)); //带进一个页数，并返回该页所要显示的信息 
	...
	function allch()
	{
		for(i=0;i<<%=pagelist3.size()%>;i++)
		{
		 ...
		 <form action="<%=basePath %>MemberManage.shtml?method=DELCO" method="post" name="pageform">
			<%
		  ...
		  <TR align="center" bgColor=#ffffff>
		    <TD id=map><%=i+1 %></TD>
			<TD id=map>
			<a title="点击查看详细信息" href="javascript:" onclick="window.showModelessDialog('<%=basePath+dir %>/member/cminfo.jsp?id=<%=pagelist2.get(0).toString()%>','newwin','dialogHeight:400px;   dialogWidth:   600px;   dialogTop:0px;   dialogLeft:   5px;   edge:   Raised;   center:   Yes;   help:   Yes;   resizable:   Yes;   status:yes;');">					
